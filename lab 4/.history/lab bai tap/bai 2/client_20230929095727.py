...
def client(host, port):  # Định nghĩa hàm client nhận host và port là tham số
...
if __name__ == "__main__":  # Kiểm tra xem chương trình có được chạy trực tiếp hay không
    ...
    port = 12355  # Gán giá trị cổng 12355 (sinh viên phải sử dụng cùng cổng với server)
    client(host, port)  # Gọi hàm client với host và port đã được định nghĩa
